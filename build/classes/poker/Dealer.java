...
public class Dealer {
    ...
    public boolean verifyStraight(ArrayList<Card> hand, Player p) {
        ...
        if (straightCounter == 5) {
            ...
            clearStraight();
            System.out.println("Chance failed! Counter: "+straightCounter);
            return false;
        }
    ...
    public void returnCommunityCards() {
        deck.push(turn);
        deck.push(river);
    ...
    public Card selectFromDeck(int value, String suit) {
        for (int i = 0; i < deck.size(); i++) {
            if (deck.get(i).getSuit().equalsIgnoreCase(suit) && deck.get(i).getValue() == value) {
    ...
    public void pickCards(Player p) {
        Scanner s = new Scanner(System.in);

        System.out.println("First card suit for " +p.getName());
        String suit1 = s.next();

        System.out.println("First card value " +p.getName());
        int value1 = s.nextInt();

        p.takeCard(selectFromDeck(value1, suit1));

        System.out.println("Second card suit " +p.getName());
        String suit2 = s.next();

        System.out.println("Second card value " +p.getName());
        int value2 = s.nextInt();

        p.takeCard(selectFromDeck(value2, suit2));
    }
    ...
    public void pickCommunity(){
        ...
        for(int i = 0; i < 3; i++){
            System.out.println("Enter card suit for flop card"+ i+1);
            suit = s.next();
            
            System.out.println("Enter card value for flop card"+ i+1);
            value = s.nextInt();
            
            flop.add(selectFromDeck(value, suit));
        }
        
        System.out.println("Enter card suit for turn");
        suit = s.next();
        System.out.println("Enter card value for turn");
        value = s.nextInt();
        
        turn = selectFromDeck(value, suit);
        
        System.out.println("Enter card suit for river");
        suit = s.next();
        System.out.println("Enter card value for river");
        value = s.nextInt();
        
        river = selectFromDeck(value, suit);
       
    }

    public void returnHand(Player p) {
        int handSize = p.getHand().size();

        for (int i = 0; i < handSize; i++) {
            deck.push(p.getHand().remove(0));
        }
